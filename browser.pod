...
will run B<help foo> when accessing a B<help://foo> uri.
...
Open the history page in the current browser window (or open a new one if no 
such window exists). The same can be achieved by opening the B<history://> 
uri. With B<!>, open the page in the sidebar (see L</"The Sidebar"> for 
details).
...
access time. The number of items in the history is given by 
L</"g:browser_history_size">.
...
F<ftplugin/browser/mappings.vim> file from the F<extra> directory is 
installed. All mappings are local to browser windows, unless otherwise 
mentioned. The following key mappings are available:
...
<CR>: Click form input, change value or submit (L</:BrowserClick>). In insert 
mode (when editing a text input), <CR> will submit the form containing the 
currently edited text input.
...
<S-Up>, <S-Down>: scroll the text in a textarea input up and down 
(L</"textareas">).
...
to google, dictionary or thesaurus, respectively.
...
L</"Mappings"> for a list of all key mappings, and L</"Settings"> for a list 
of all variables affecting the browser behaviour.
...
:BrowserHistory, :History - open the global history as a browser page 
(L</":History[!]">)
...
Version 1.0. For vim version 6.3
